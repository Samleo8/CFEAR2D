...
| `benchmark` | Run benchmarking using [rpg-evaluator](https://github.com/uzh-rpg/rpg_trajectory_evaluation) AFTER main program is run. Outputs into `./results/<DATASET_ID>/poses/saved_results` and `.../plots` folder. NOTE: Currently only allows for benchmarking on full sequence. | `./scripts/startBenchmark.sh` |
