/*-------------------------------------------------------------------------------------------------------------
EE5003 PROJECT: Radar Odometry -- CFEAR Algorithm Implementation
AUTHOR : LIU CEN - NUS ECE
VERSION: Multiple Keyframe Registration + Constant Velocity Model
-------------------------------------------------------------------------------------------------------------*/
...
int main(int argc, char** argv) {
	...
	const std::string img_path = "C:/Users/liucen/Documents/EE_Project/Dataset/2019-01-10-14-36-48-radar-oxford-10k-partial/radar/*.png";
	const std::string save_path = "C:/Users/liucen/Documents/EE_Project/Dataset/2019-01-10-14-36-48-radar-oxford-10k-partial/processed_output/";
	...
	//// MulRan Radar Dataset
	//const std::string img_path = "C:/Users/liucen/Documents/EE_Project/Dataset/MulRan_Riverside01/polar_oxford_form/*.png";
	//const std::string save_path = "C:/Users/liucen/Documents/EE_Project/Dataset/MulRan_Riverside01/processed_output/";
	...
	const int S = 3; // Number of keyframes
	...
	const double yaw_keyframe = 5.0 * (M_PI / 180); // Admit current frame into keyframes if estimated rotation exceeds this value [rads]
	...
	for (int n = 0; n < paths.size(); n++) {
		...
		const int max_num_iter_out = 50, max_num_iter_in = 1;

		// Build and solve the problem
		ceres::Problem odometry;
		ceres::Solver::Options options;
		options.max_num_iterations = max_num_iter_in;
		...
		const double iter_th = 1e-6;

		for (int iter_out = 0; iter_out < max_num_iter_out; iter_out++) {
			// Add the residuals
			for (int m = 0; m < S; m++) {
				for (int i = 0; i < current_frame.means.size(); i++) {
					Eigen::Vector3d mu_ti(current_frame.means[i]);

					int j = FindNeighborInWorld(current_frame, keyframes[m], T_wt.data(), T_wk_queue[m].data(), i, grid_r, yaw_max);
					if (j != -1) {
						...
						auto block_ID = odometry.AddResidualBlock(radar_cost_func, huber_loss_func, T_wt.data());
						block_IDs.push_back(block_ID);
					}
				}
			}

			//cout << iter_out << " " << block_IDs.size() << endl;

			ceres::Solve(options, &odometry, &summary);

			// Remove all residual blocks from the problem after get the solution
			if (block_IDs.size() > 0) {
				for (int i = 0; i < block_IDs.size() - 1; i++) {
					odometry.RemoveResidualBlock(block_IDs[i]);
				}
				block_IDs.clear(); // Empty the block_IDs vector after remove the residual blocks
			}

			const double diff_cost = abs((summary.initial_cost - summary.final_cost) / summary.initial_cost);
			if (diff_cost < iter_th) {
				break;
			}
		}
